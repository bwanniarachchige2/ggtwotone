# -----------------------------------------------------------------------------
# Contrast backend can be selected with the ggtwotone.contrast_method option.
# Supported values are "WCAG" and "APCA".
# Or let highlight_colors(contrast_method = "auto") choose per polarity.
# -----------------------------------------------------------------------------
...
#'   (`auto_bias`)
#' - NEW: `contrast_method = "auto"` picks APCA for dark-on-light
#'   and WCAG for light-on-dark
#'
#' Creates n colors by:
...
#' @param contrast_bg Required contrast vs background (WCAG ratio or APCA Lc
#'   depending on backend). Default 4.5.
#' @param contrast_base Required contrast vs base_color (set 0 to skip).
#'   Default 3.0.
#' @param min_deltaE Minimum CIEDE2000 separation between selected colors.
#'   Default 35.
#' @param hue_targets "auto", "anchored", "spread", or numeric vector of
#'   hue angles (0-360).
#' @param anchor_band Degrees around base hue to sample when
#'   `hue_targets = "anchored"`. Default 180.
#' @param hcl_L_range Allowed HCL lightness range. Default `c(25, 75)`.
#' @param hcl_C_range Allowed HCL chroma range. Default `c(40, 90)`.
#' @param min_hue_sep Minimum separation in degrees between chosen hues.
#'   Default 35.
#' @param n_candidates Number of random candidates before selection.
#'   Default 100000.
#' @param relax Progressively relax `min_hue_sep` then `min_deltaE`
#'   if infeasible. Default TRUE.
#' @param quiet Suppress relaxation messages. Default TRUE.
#' @param repel_from Character vector among `c("background", "base")`
#'   to avoid those hue neighborhoods.
#' @param repel_band Degrees excluded around the repelled hues
#'   (`+/-` band). Default 50.
#' @param auto_bias Logical; if TRUE and bg/base are far apart,
#'   bias sampling toward opponent hues.
#' @param bias_width Width (degrees) of opponent sampling window when
#'   `auto_bias` applies. Default 120.
#' @param bias_frac Fraction of samples drawn from the biased window when
#'   `auto_bias` applies. Default 0.8.
#' @param contrast_method `"option"` (use global option, default)
#'   or `"auto"` (pick WCAG/APCA per polarity).
#'
#' @return Character vector of hex colors (length <= n) with
#'   an `info` attribute.
#'
#' @examples
#' highlight_colors(
...
highlight_colors <- function(
  ...
  if (is.na(bg_hex) || is.na(base_hex)) {
    stop("Invalid `background` or `base_color`.")
  }
  ...
  if (identical(contrast_method, "auto")) {
    ...
    on.exit(
      options(ggtwotone.contrast_method = old_backend),
      add = TRUE
    )
  }

  # guardrail: if WCAG & requested contrast vs bg is only
  # possible on the light side
  if (identical(toupper(selected_backend), "WCAG")) {
    ...
    max_if_lighter <- 1.05 / (bg_linL + 0.05)

    if (contrast_bg > max_if_lighter) {
      hcl_L_range <- c(
        min(5, hcl_L_range[1]),
        max(20, min(40, hcl_L_range[2]))
      )
      hcl_C_range <- c(0, max(40, hcl_C_range[2]))

      if ("background" %in% repel_from) {
        repel_from <- setdiff(repel_from, "background")
      }
      ...
      if (!quiet) {
        message(
          "Strict contrast vs this background forces near-black highlights; ",
  ...
  hue_dist_ok <- function(h1, h2, thr) {
    circ_dist(h1, h2) >= thr
  }

  base_h <- tryCatch(
    farver::decode_colour(base_color, to = "hcl")[, 1],
    error = function(e) {
      NA_real_
    }
  )
  ...
  bg_h <- tryCatch(
    farver::decode_colour(background, to = "hcl")[, 1],
    error = function(e) {
      NA_real_
    }
  )
  ...
  if (is.character(hue_targets)) {
    ...
    H <- as.numeric(
      sample(hue_targets %% 360, size = n_candidates, replace = TRUE)
    )
    mode_used <- "numeric"
  } else {
    stop(
      '`hue_targets` must be "auto", "anchored", ',
      '"spread", or numeric angles.'
    )
  }
  ...
  if (n_in_gamut < n) {
    stop("Too few in-gamut candidates; widen ranges or increase n_candidates.")
  }

  apply_repel <- function(hex_vec, repel_deg) {
    if (!length(repel_from) || repel_deg <= 0) {
      return(hex_vec)
    }
  ...
  repel_grid <- if (length(repel_from)) {
    c(repel_band, seq(max(10, repel_band - 10), 10, by = -10))
  } else {
    0
  }
  ...
  for (rb in unique(repel_grid)) {
    ...
    base_steps <- if (contrast_base > 0) {
      unique(c(contrast_base, 4.5, 3.2, 3.0, 2.8, 2.6, 2.4))
    } else {
      0
    }
  ...
  if (!found) {
    stop(
      "No candidates pass contrast filters even after relaxation.\n",
  ...
  if (
    ...
    message(
      sprintf(
        paste0(
          "Effective settings: repel_band = %d deg, ",
          "contrast_bg = %.1f, contrast_base = %.1f"
        ),
        used_repel,
        used_bg,
        used_base
  ...
  if (length(repel_from)) {
    ...
    if (!length(cand_hex)) {
      stop("All candidates filtered by repel bands; reduce `repel_band`.")
    }
  }
  ...
  if (!found) {
    ...
    message(
      sprintf(
        "Relaxed contrasts used: base = %.1f, background = %.1f",
        contrast_base_eff,
        contrast_bg_eff
      )
    )
  ...
  while (length(chosen) < n) {
    ...
    minde <- sapply(
      left,
      function(i) {
        min(sapply(chosen, function(j) DE_pair(i, j)))
      }
    )
    ...
    if (!length(eligible)) {
      ...
      if (cur_min_hsep > 12) {
        cur_min_hsep <- cur_min_hsep - 5
        if (!quiet) {
          message("Relax: min_hue_sep -> ", cur_min_hsep)
        }
        next
      }

      if (cur_min_deltaE > 26) {
        cur_min_deltaE <- cur_min_deltaE - 2
        if (!quiet) {
          message("Relax: min_deltaE -> ", cur_min_deltaE)
        }
        next
      }
  ...
  attr(res, "info") <- list(
    ...
    auto_bias = auto_bias,
    contrast_method = if (identical(contrast_method, "auto")) {
      selected_backend
    } else {
      getOption("ggtwotone.contrast_method", "WCAG")
    }
  )
