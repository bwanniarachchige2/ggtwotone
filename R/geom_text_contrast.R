GeomTextContrast <- ggplot2::ggproto("GeomTextContrast", ggplot2::Geom,
  ...
  draw_panel = function(self, data, panel_params, coord,
                        ...
                        base_colour = NULL) {
    # coalesce naming
    if (is.null(base_colour) && !is.null(base_colour))
    ...
    bg_is_light <- function(bg) {
      colorspace::contrast_ratio(bg, "#000000") >=
        colorspace::contrast_ratio(bg, "#FFFFFF")
    }
    ...
    data$colour <- vapply(seq_len(n), function(i) {
      ...
      if (is.na(bg_i) || is.null(bg_i)) bg_i <- "#FFFFFF"
      ...
      cp <- tryCatch(
        adjust_contrast_pair(
          base_i,
          contrast = contrast, method = method, background = bg_i
        ),
      ...
      pick <- NULL
      if (prefer_dark) {
        if (is.finite(crD) && crD >= contrast) {
          pick <- dark
        } else if (is.finite(crL) && crL >= contrast) {
          pick <- light
        }
      } else {
        if (is.finite(crL) && crL >= contrast) {
          pick <- light
        } else if (is.finite(crD) && crD >= contrast) {
          pick <- dark
        }
      }
      if (is.null(pick)) {
        if (is.finite(crL) && is.finite(crD)) {
          pick <- if (crL >= crD) light else dark
        } else if (is.finite(crL)) {
          pick <- light
        } else if (is.finite(crD)) {
          pick <- dark
        } else {
          pick <- if (colorspace::contrast_ratio("#FFFFFF", bg_i) >=
                        colorspace::contrast_ratio("#000000", bg_i)) {
            "#FFFFFF"
          } else {
            "#000000"
          }
...
#'
#' Automatically adjusts text colour for readability on varying background
#' colours using WCAG or APCA contrast.
#'
#' @rdname geom_text_contrast
#' @inheritParams ggplot2::geom_text
#' @param background A character vector of background fill colours (hex codes),
#' used for contrast computation.
...
#' @param contrast Threshold to ensure between text and background
#' (defaults to 4.5).
#' @export
#'
#' @examples
