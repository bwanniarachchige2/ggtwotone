GeomCurveDual <- ggplot2::ggproto(
  ...
  draw_panel = function(
    ...
    if (!"colour1" %in% names(coords)) {
      coords$colour1 <- "white"
    }
    if (!"colour2" %in% names(coords)) {
      coords$colour2 <- "black"
    }

    # fallback to params if aesthetics missing/NA
    if (!"curvature" %in% names(coords) || all(is.na(coords$curvature))) {
      coords$curvature <- curvature
    }
    if (!"angle" %in% names(coords) || all(is.na(coords$angle))) {
      coords$angle <- angle
    }
    if (!"ncp" %in% names(coords) || all(is.na(coords$ncp))) {
      coords$ncp <- ncp
    }
