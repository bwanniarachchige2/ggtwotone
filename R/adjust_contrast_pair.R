...
#'
#' Given a base color and a background, generate a pair of colors
#' (light and dark)
#' with sufficient perceptual contrast using WCAG or APCA methods.
#'
#' @param color A base color, as a hex string or valid R color name
#' (e.g., "#6699CC", "darkred").
#' @param contrast Minimum desired contrast ratio (default is 4.5).
#' @param method Contrast method: "WCAG", "APCA", or "auto" to try both.
#' @param background Background color, as a hex string or valid R color name
#' (default: "#FFFFFF").
...
adjust_contrast_pair <- function(color, contrast = 4.5, method = "auto",
  ...
  if (!is.character(color) || length(color) != 1 || is.na(color)) {
    ...
    return(list(light = "#FFFFFF", dark = "#000000",
                contrast = NA, method = method))
  }
  ...
  if (is.na(background)) {
    if (!quiet) warning(sprintf("Invalid background color: %s",
                                orig_background))
  ...
  if (is.na(color)) {
    if (!quiet) warning(sprintf("Could not convert base color %s to HCL.",
                                orig_color))
    return(list(light = "#FFFFFF", dark = "#000000", contrast = NA,
                method = method))
  }
  ...
  get_contrast <- function(fg, bg, method) {
    result <- tryCatch({
      ...
      warning(sprintf("computation failed for %s vs %s [%s]: %s", fg, bg,
                      method, e$message))
      NA
    })
    result
  ...
  for (m in contrast_methods) {
    ...
    if (!is.na(max_contrast) && max_contrast >= contrast) {
      best_result <- list(light = best_light, dark = best_dark,
                          contrast = max_contrast, method = m)
      break
    }
  }

  if (is.null(best_result)) {
    warning(sprintf("No sufficient contrast found for base color %s.
                    Using fallback colors.", color))
    return(list(light = "#FFFFFF", dark = "#000000", contrast = NA,
                method = method))
  }
